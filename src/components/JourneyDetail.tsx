...
export const JourneyDetail: React.FC<JourneyDetailProps> = ({
  ...
  const handleDeleteLog = async (logId: string) => {
    ...
    try {
      ...
      setJourney(prevJourney => ({
        ...
        totalHoursLogged: prevJourney.totalHoursLogged - (logToDelete?.hoursWorked || 0)
      }));
  ...
  const daysSinceStart = (new Date().getTime() - new Date(journey.createdAt).getTime()) / (1000 * 60 * 60 * 24);
  console.log("Days since start:", daysSinceStart);
  console.log("Total hours logged:", journey.totalHoursLogged);
  console.log("Initial hours:", journey.initialHours);
  const averageHoursPerDay = journey.logs.length > 0
    ? ((journey.totalHoursLogged - journey.initialHours) / Math.ceil(daysSinceStart)).toFixed(1)
    : 0;
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        onClick={() => setShowLogForm(true)}>Add Log</Button>
      </div>
      ...
      <Card className="p-4 bg-gray-800">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div>
            ...
            <p className="text-white font-semibold">{remainingHours} hours</p>
          </div>
          <div>
            <p className="text-gray-400">Average per Day</p>
            <p className="text-white font-semibold">{averageHoursPerDay} hours</p>
          </div>
        </div>
      </Card>
